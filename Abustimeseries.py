...
#bus ids to match
a_ids = [7181, 7151, 17625, 15187, 20283]

#file merging and cleaning
A = riders.merge(bus[['id', 'routeName']], left_on='bus_id', right_on='id', how='left')
A = A[A['bus_id'].isin(a_ids)]
A = A.drop(columns=['bus_id', 'id', 'routeName'])


A = (
    A.groupby('time')['ridership_count']
    .mean()
    .reset_index()
)
...
A['timestamp'] = start_time + pd.to_timedelta(A['time'], unit='m')
...
riders_all = riders_all[riders_all['bus_id'].isin(a_ids)]
...
#plt.plot(A.index, A'ridership_count'], marker='o', color ='green')
A_resampled = A.resample('5min').mean()  # average every x minutes
plt.plot(A_resampled.index, A_resampled['ridership_count'], color='blue', linewidth=2)
...
plt.title("Ridership Time Series for A Buses")
plt.xlabel("Time")
